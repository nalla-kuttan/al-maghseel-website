        ...
        <motion.section id="contact" className="bg-brand-50" {...fadeUp}>
            ...
            <div className="border-y border-brand-100 bg-white">
                <div className="mx-auto grid max-w-7xl gap-5 px-4 py-7 md:grid-cols-[1fr_1fr_1fr_1fr] md:items-center">
                    <h2 className="font-heading text-2xl font-black leading-tight text-gray-950">
                        Need A/C Help?<br />We&apos;re Just One Call Away!
                    </h2>
                    <a href={`tel:${COMPANY.phone}`} className="flex items-center gap-4 border-brand-100 md:border-l md:pl-7">
                        ...
                        <span><span className="block text-xs font-black uppercase text-gray-600">Call Us Now</span><span className="text-xl font-black text-brand-900">{COMPANY.phone.replace("+971", "0")}</span></span>
                    </a>
                    <a href={`https://wa.me/${COMPANY.whatsappDigits}`} className="flex items-center gap-4 border-brand-100 md:border-l md:pl-7">
                        <span className="grid h-11 w-11 place-items-center rounded-full bg-[#07973f] text-white"><MessageCircle className="h-5 w-5" /></span>
                        <span><span className="block text-xs font-black uppercase text-gray-600">WhatsApp Us</span><span className="text-xl font-black text-brand-900">{COMPANY.phone.replace("+971", "0")}</span></span>
                    </a>
            ...
            <div className="mx-auto max-w-7xl px-4 py-16">
                <div className="grid lg:grid-cols-2 gap-10">
                    <div>
                        <div className="text-xs font-black uppercase tracking-[0.18em] text-brand-800">Get in Touch</div>
                        <h2 className="mt-2 text-3xl md:text-4xl font-heading font-black text-gray-950">Contact Us</h2>
                        ...
                        <div className="mt-8 space-y-4">
                            <a href={`tel:${COMPANY.phone}`} className="group flex items-center gap-4 rounded-md border border-slate-100 bg-white p-5 shadow-sm transition-all hover:border-brand-100 hover:shadow-md">
                            ...
                            <a href={`mailto:${COMPANY.email}`} className="group flex items-center gap-4 rounded-md border border-slate-100 bg-white p-5 shadow-sm transition-all hover:border-brand-100 hover:shadow-md">
                                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-brand-50 text-brand-800 transition-colors group-hover:bg-brand-900 group-hover:text-white">
                        ...
                        <div className="mt-8 flex gap-4">
                            <button
                                ...
                                className="flex-1 rounded bg-brand-900 px-6 py-4 text-center text-xs font-extrabold uppercase tracking-wide text-white shadow-lg shadow-brand-900/20 transition-all hover:bg-brand-800 active:scale-[0.98]"
                            >
                                Book Appointment
                            </button>
                            <a
                                href={`https://wa.me/${COMPANY.whatsappDigits}`}
                                className="flex-1 rounded border-2 border-brand-200 px-6 py-4 text-center text-xs font-extrabold uppercase tracking-wide text-brand-900 transition-all hover:bg-white active:scale-[0.98]"
                            >
                                WhatsApp Chat
                            </a>
                        </div>
